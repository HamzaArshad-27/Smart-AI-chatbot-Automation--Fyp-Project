...
# Setup Django environment so it can be run directly using `python products_import.py`
import django
...
PRODUCTS_DATA = {
    ...
    'sports-outdoors': [
        ...
        ("Inflatable Kayak 2 Person", "1507038641620-7f02a4282cf3", 299.99, 399.99, "14.0", "320x90 cm"),
...
def run():
    ...
    for cat_slug, products in PRODUCTS_DATA.items():
        cat_slug_cleaned = cat_slug.strip().lower()
        cat_name = cat_slug_cleaned.replace('-', ' ').title()
        description = category_descriptions.get(cat_slug_cleaned, f"Premium quality {cat_name} products.")
        
        category, created = Category.objects.get_or_create(
            slug=cat_slug_cleaned,
            defaults={
                'name': cat_name,
